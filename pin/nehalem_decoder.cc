...
Decoder::Instr::Instr(INS _ins) : ins(_ins), numLoads(0), numInRegs(0), numOutRegs(0), numStores(0)
{
    ...
    for (uint32_t op = 0; op < numOperands; op++)
    {
        ...
        { 
            ...
            uint32_t reg = INS_OperandReg(ins, op);
...
{
    LOG_PRINT_WARNING("Unhandled case: %s | %s | loads=%d stores=%d inRegs=%d %s outRegs=%d %s",
                      desc, INS_Disassemble(instr.ins).c_str(),
                      instr.numLoads, instr.numStores,
                      instr.numInRegs, regsToString(instr.inRegs, instr.numInRegs).c_str(),
...
void Decoder::emitLoad(Instr& instr, uint32_t idx, MicroOpVec& uops, uint32_t destReg)
{
    ...
    uop.rs[1] = indexReg;
    uop.rd[0] = destReg;
    uop.type = MicroOp::LOAD;
    uop.portMask = PORT_2;
    uops.push_back(uop);
    // FIXME: The interface should support in-place grow...
}

void Decoder::emitStore(Instr& instr, uint32_t idx, MicroOpVec& uops, uint32_t srcReg)
{
    ...
    addrUop.type = MicroOp::STORE_ADDR;
    ...
    uop.rs[1] = srcReg;
    uop.portMask = PORT_4;
    uop.type = MicroOp::STORE;
    uops.push_back(uop);
}


void Decoder::emitLoads(Instr& instr, MicroOpVec& uops)
{
    for (uint32_t i = 0; i < instr.numLoads; i++)
        emitLoad(instr, i, uops);
}

void Decoder::emitStores(Instr& instr, MicroOpVec& uops)
{
    for (uint32_t i = 0; i < instr.numStores; i++)
        emitStore(instr, i, uops);
}

void Decoder::emitFence(MicroOpVec& uops, uint32_t lat)
{
    MicroOp uop;
    uop.clear();
    uop.lat = lat;
    uop.portMask = PORT_4; // to the store queue
    uop.type = MicroOp::FENCE;
    uops.push_back(uop);
}

void Decoder::emitExecUop(uint32_t rs0, uint32_t rs1, uint32_t rd0, uint32_t rd1,
                          MicroOpVec& uops, uint32_t lat, uint8_t ports, uint8_t extraSlots)
{
    MicroOp uop;
    uop.clear();
    uop.rs[0] = rs0;
    ...
    uop.rd[1] = rd1;
    uop.lat = lat;
    uop.type = MicroOp::GENERAL;
    uop.portMask = ports;
    uop.extraSlots = extraSlots;
    uops.push_back(uop);
}

void Decoder::emitBranchUop(uint32_t rs0, uint32_t rs1, uint32_t rd0, uint32_t rd1,
                            MicroOpVec& uops, uint32_t lat, uint8_t ports)
{
    ...
    uop.rd[1] = rd1;
    uop.lat = lat;
    uop.type = MicroOp::BRANCH;
    uop.portMask = ports;
    uops.push_back(uop);
}

void Decoder::emitBasicMove(Instr& instr, MicroOpVec& uops, uint32_t lat, uint8_t ports)
{
    if (instr.numLoads + instr.numInRegs > 1 || instr.numStores + instr.numOutRegs != 1)
    {
        reportUnhandledCase(instr, "emitBasicMove");
    }
    // Note that we can have 0 loads and 0 input registers.
    // In this case, we are loading from an immediate, and we set the input register to 0
    //  so there is no dependence
    uint32_t inReg = (instr.numInRegs == 1) ? instr.inRegs[0] : 0;
    if (!instr.numLoads && !instr.numStores)
    {   //reg->reg
        emitExecUop(inReg, 0, instr.outRegs[0], 0, uops, lat, ports);
    }
    else if (instr.numLoads && !instr.numStores)
    {   //mem->reg
...
void Decoder::emitXchg(Instr& instr, MicroOpVec& uops)
{
    if (instr.numLoads)
    {   
        // mem <-> reg
        ...
        // FIXME: Can't we do this with 3 Uops? Seems like it has an extra Exec-Uop 
        emitLoad(instr, 0, uops);
        ...
        {
            // xchg has an implicit lock prefix (TODO: Check we don't introduce two fences...)
            // FIXME: Why does a fence take 14 cycles latency?
            emitFence(uops, 14);
        }
    }
    else // (!inst.numLoads)
...
void Decoder::emitConditionalMove(Instr& instr, MicroOpVec& uops, uint32_t lat, uint8_t ports)
{
...
void Decoder::emitCompareAndExchange(Instr& instr, MicroOpVec& uops)
{
    ...
    // LOG_PRINT("%d %d %d | %d %d %d", srcRegs[0], srcRegs[1], srcRegs[2], dstRegs[0], dstRegs[1], dstRegs[2]);
    ...
    // Compare destination (first operand) w/ RAX.
    // If equal, copy source (second operand) into destination and set the zero flag;
    // Else copy destination into RAX
    if (!instr.numLoads)
    {
        assert(instr.numStores == 0);
        // 2 swaps, implemented in 2 stages: first, and all sources with rflags.zf; then or results pairwise.
        // This is pure speculation, but matches uops required.
    ...
    {
        assert(instr.numLoads == 1);
        assert(instr.numStores == 1);
        // 6 uops (so 3 exec), and critical path is 4 (for rax), GO FIGURE
        emitExecUop(srcRegs[0], rax, REG_EXEC_TEMP, rflags, uops, 2, PORTS_015);
        // Let's assume we can do a fancy conditional store
        emitExecUop(srcRegs[1], rflags, dstRegs[0], 0, uops, 2, PORTS_015);
        emitExecUop(srcRegs[2], REG_EXEC_TEMP, dstRegs[1] /*rax*/, 0, uops, 2, PORTS_015); //likewise
    }

    // NOTE: While conceptually srcRegs[0] == dstRegs[0], when it's a memory location,
    //  they map to different temporary regs

    emitStores(instr, uops);
}

void Decoder::emitBasicOp(Instr& instr, MicroOpVec& uops, uint32_t lat, uint8_t ports, uint8_t extraSlots, bool reportUnhandled)
{
    emitLoads(instr, uops);
...
void Decoder::emitChainedOp(Instr& instr, MicroOpVec& uops, uint32_t numUops, uint32_t* latArray, uint8_t* portsArray)
{
    emitLoads(instr, uops);
    ...
    assert(srcs + 1 >= numUops);
    // note equality is not necessary in case one or more operands are immediates
...
void Decoder::emitConvert2Op(Instr& instr, MicroOpVec& uops, uint32_t lat1, uint32_t lat2, uint8_t ports1, uint8_t ports2)
{
    if (instr.numStores > 0 || instr.numLoads > 1 || instr.numOutRegs != 1
            || instr.numLoads + instr.numInRegs != 1)
    {
...
void Decoder::emitMul(Instr& instr, MicroOpVec& uops)
{
    uint32_t dsts = instr.numStores + instr.numOutRegs;
    if (dsts == 3)
    {
...
void Decoder::emitDiv(Instr& instr, MicroOpVec& uops)
{
    uint32_t srcs = instr.numLoads + instr.numInRegs;
    uint32_t dsts = instr.numStores + instr.numOutRegs;
    ...
    switch (width)
    {
     ...
     case 64:
         ...
         break;
     default:
         LOG_PRINT_ERROR("emitDiv: Invalid reg size");
    }
...
bool Decoder::dropRegister(uint32_t targetReg, uint32_t* regs, uint32_t& numRegs)
{
...
{
    bool dropIn = Decoder::dropRegister(REG_RSP, instr.inRegs, instr.numInRegs);
    bool dropOut = Decoder::dropRegister(REG_RSP, instr.outRegs, instr.numOutRegs);
    if (!dropIn && !dropOut)
        reportUnhandledCase(instr, "dropStackRegister (no RSP found)");
    else // (dropIn || dropOut)
        reportUnhandledCase(instr, "dropStackRegister (RSP found)");
}


bool Decoder::decode(INS ins, MicroOpVec& uops)
{
    uint32_t initialUops = uops.size();
    ...
    case XC(DATAXFER):
        ...
        case XO(XCHG):
            ...
            break;
        default:
            // TODO: MASKMOVQ, MASKMOVDQ, MOVBE (Atom only),
            //       MOVNTxx variants (nontemporal), MOV_CR and MOV_DR (privileged?),
            //       VMOVxxxx variants (AVX)
            inaccurate = true;
            emitBasicMove(instr, uops, 1, PORTS_015);
        }
        break;
    ...
    case XC(SHIFT):
        {
            ...
            {
                // SHRD takes 4 cycles total, SHLD takes 3
                uint32_t lats[] = {2, opcode == XO(SHLD)? 1u : 2u};
                uint8_t ports[] = {PORTS_015, PORTS_015};
                emitChainedOp(instr, uops, 2, lats, ports);
            }
            else // (opcode != XO(SHLD) && opcode != XO(SHRD))
            {
                // SHR, SHL, SAR are 1 cycle
                uint32_t opLat = 1;
                emitBasicOp(instr, uops, opLat, PORT_0 | PORT_5);
            }
        }
        break;
    case XC(DECIMAL):
        // pack/unpack BCD, these seem super-deprecated
        {
            uint32_t opLat = 1;
            switch (opcode)
            {
            ...
            case XO(AAM):
                ...
                break;
            default:
                LOG_PRINT_ERROR("Invalid opcode for this class");
            }
            emitBasicOp(instr, uops, opLat, PORTS_015);
        }
        break;
    ...
    case XC(SEMAPHORE):
        // Atomic ops, these must involve memory
    ...
    case XC(SSE):
        {
            ...
            case XO(PCMPGTQ): // weird, only packed comparison that's done differently
                lat = 3;
                ports = PORT_1;
                break;
    ...
    case XC(PCLMULQDQ):
        // CLMUL extension (carryless multiply, generally related to AES-NI)
        break;

    case XC(XSAVE):
    case XC(XSAVEOPT):
        // Hold your horses, it's optimized!! (AVX)
        break;

    // Control flow ops (branches, jumps)
    case XC(COND_BR):
        // We model all branches with a latency of 1.
        assert(instr.numLoads == 0 && instr.numStores == 0);
        assert(instr.numInRegs == 2 && instr.numOutRegs == 1);
        assert(instr.inRegs[0] == REG_RIP && instr.inRegs[1] == REG_RFLAGS && instr.outRegs[0] == REG_RIP);
        emitBranchUop(REG_RIP, REG_RFLAGS, REG_RIP, 0, uops, 1, PORT_5); 
        break;

    case XC(UNCOND_BR):
        // We model all jumps with a latency of 1.
        // Far jumps are really expensive, but they should be exceedingly rare
        //  (from Intel's manual, they are used for call gates, task switches, etc.)
    ...
    case XC(STRINGOP):
        ...
        case XO(CMPSQ):
            ...
            // Agner's tables say all exec uops can go anywhere,
            //  but I'm betting the comp op only goes in port5
            ...
            break;
        default:
            // SCAS and other dragons I have not seen yet
            inaccurate = true;
        }
        break;
    ...
    // Stuff not even the Intel guys know how to classify
    case XC(MISC):
        switch (opcode)
        {
        case XO(LEA):
            ...
            break;
        case XO(PAUSE):
            // Pause is weird. It takes 9 cycles, issues 5 uops
            // (to be treated like a complex instruction and put a wrench on the decoder?),
            // and those uops are issued to PORT_015. No idea about how individual uops are sized,
            // but in ubenchs I cannot put even an ADD
        ...
        case XO(LFENCE):
        case XO(SFENCE):
            // All memory operations follow TSO. So, no need for load or store fences
            break;
    ...
    default:
        inaccurate = true;
        // LOG_PRINT_ERROR("Invalid instruction category");
    }

    // Try to produce something approximate...
    if (uops.size() - initialUops == isLocked? 1 : 0)
    { 
        // If it's locked, we have the initial fence for an empty instr
        emitBasicOp(instr, uops, 1, PORTS_015, 0, false /* don't report unhandled cases */);
        inaccurate = true;
    }

    // NOTE: REP instructions are unrolled by PIN, so they are accurately simulated
    // (they are treated as predicated in Pin)
    ...
    if (isLocked)
    {
        // Locked ops introduce an additional uop and cache locking takes 14 cycles/instr
        // per the perf counters; latencies match with 9 cycles of fence latency
        emitFence(uops, 9);
    }
...
// FIXME: Why is mirco-op fusion not implemented?
...
bool Decoder::decodeFused(INS ins, MicroOpVec& uops)
{
    // assert(canFuse(ins)); // this better be true
    Instr instr(ins);
    Instr branch(INS_Next(ins));

    // Instr should have 2 inputs (regs/mem), and 1 output (rflags),
    //  and branch should have 2 inputs (rip, rflags) and 1 output (rip)

    if (instr.numOutRegs != 1  || instr.outRegs[0] != REG_RFLAGS  ||
        branch.numOutRegs != 1 || branch.outRegs[0] != REG_RIP    ||
        branch.numInRegs != 2  || branch.inRegs[0] != REG_RIP     || branch.inRegs[1] != REG_RFLAGS)
    {
        reportUnhandledCase(instr, "decodeFused");
        reportUnhandledCase(branch, "decodeFused");
    }
    else
    {
...
Instruction* Decoder::decodeInstruction(INS ins)
{
#ifdef TARGET_PROFILING
    xed_iform_enum_t approxOpcode = XED_IFORM_INVALID;
    ...
    MicroOpVec uopVec;
    __attribute__((unused)) bool inaccurate = Decoder::decode(ins, uopVec);
    ...
    if (inaccurate)
    {
        ...
        if (err != XED_ERROR_NONE)
            LOG_PRINT_ERROR("xed_decode failed: %s", xed_error_enum_t2str(err));
        else
            approxOpcode = xed_decoded_inst_get_iform_enum(&xedd);
        // LOG_PRINT("Approx decoding: %s", INS_Disassemble(ins).c_str());
    }
#endif

    // Allocate
    MicroOp* uopArray = new MicroOp[uopVec.size()];
    for (uint32_t i = 0; i < uopVec.size(); i++)
        uopArray[i] = uopVec[i];
    Instruction* instruction = new Instruction(INS_Address(ins), INS_Size(ins),
                                               uopVec.size(), uopArray);

    // Initialize ooo part

#ifdef TARGET_PROFILING
    globalIdxLock.acquire();
    instructionArray[globalIdx++] = instruction;
    assert(globalIdx < MAX_INSTRUCTIONS);
    instruction->setCount(0);
    instruction->setApproxOpcode((uint32_t) approxOpcode);
    globalIdxLock.release();
#endif

    return instruction;
}

BasicBlock* Decoder::decodeBasicBlock(BBL bbl)
{
    ...
    for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
    {
        ...
        {
            inaccurate = Decoder::decodeFused(ins, uopVec);
        ...
        {
            inaccurate = Decoder::decode(ins, uopVec);
    ...
    // Instr predecoder and decode stage modeling; we assume clean slate between BBLs,
    //  which is typical because optimizing compilers 16B-align most branch targets
    //  (and if it doesn't happen, the error introduced is fairly small)
    ...
    for (uint32_t i = 0; i < instrs; i++)
    {
        ...
        predecCycle[i] = pcyc;
        // LOG_PRINT("PREDEC %2d: 0x%08lx %2d %d %d %d",
        //           i, instrAddr[i], instrBytes[i], instrUops[i], block, predecCycle[i]);
    }
    ...
    for (uint32_t i = 0; i < instrs; i++)
    {
        ...
        if ((simple && dsimple + dcomplex == 4) || (!simple && dcomplex == 1))
        { 
            // Do: (!simple /*&& dcomplex == 1*/) to be conservative?
            dcyc++;
            dsimple = 0;
            dcomplex = 0;
        }
        ...
        // LOG_PRINT("   DEC %2d: 0x%08lx %2d %d %d %d (%d %d)",
        //           i, instrAddr[i], instrBytes[i], instrUops[i], simple, dcyc, dcomplex, dsimple);
    ...
    for (uint32_t i = 0; i < uopVec.size(); i++)
        uopArray[i] = uopVec[i];
    BasicBlock* basicBlock = new BasicBlock(instrs, BBL_Address(bbl), bytes,
                                            uopVec.size(), uopArray);

#ifdef TARGET_PROFILING
    globalIdxLock.acquire();
    basicBlockArray[globalIdx++] = &basic_block;
    assert(globalIdx < MAX_BASIC_BLOCKS);
    basicBlock->count = 0;
    basicBlock->approxOpcodes = approxInstrs ? new std::vector<uint32_t>(approxOpcodes) : NULL;
    globalIdxLock.release();
#endif
...
#ifdef TARGET_PROFILING
void Decoder::profileInstruction(uint64_t insIdx)
{
    assert(instructionIdx < MAX_INSTRUCTIONS);
    __sync_fetch_and_add(&instructionArray[insIdx].count, 1);
}

void Decoder::profileBasicBlock(uint64_t bblIdx)
{
    assert(bblIdx < MAX_BASIC_BLOCKS);
    __sync_fetch_and_add(&basicBlockArray[bblIdx].count, 1);
}

void Decoder::dumpInstructionProfile()
{
    ...
    for (uint32_t i = 0; i < globalIdx; i++)
    {
        Instruction& instruction = *instructionArray[i];
        xed_iform_enum_t op = (xed_iform_enum_t) instruction.approxOpcode;
        if (op != XED_IFORM_INVALID)
            approxOpcodeCount[op] ++;
    }

    dumpApproxInstrProfile(approxOpcodeCount);
    
    // Ins profile
    std::ofstream out("ins_profile.stats");
    for (uint32_t i = 0; i < globalIdx; i++)
        out << std::setw(8) << i << std::setw(8) << instructionArray[i]->count << std::endl;
    out.close();
}

void Decoder::dumpBasicBlockProfile()
{
    uint32_t numOpcodes = xed_iform_enum_t_last() + 1;
    uint64_t approxOpcodeCount[numOpcodes];
    for (uint32_t i = 0; i < numOpcodes; i++)
    ...
    for (uint32_t i = 0; i < globalIdx; i++)
    {
        BasicBlock& basicBlock = *basicBlockArray[i];
        if (basicBlock.approxOpcodes)
            for (uint32_t& op : *basicBlock.approxOpcodes)
                approxOpcodeCount[op] += basicBlock.count;
    }
    
    dumpApproxInstrProfile(approxOpcodeCount);

    // Bbl profile
    std::ofstream out("bbl_profile.stats");
    for (uint32_t i = 0; i < globalIdx; i++)
        out << std::setw(8) << i << std::setw(8) << basicBlockArray[i]->count << std::endl;
    out.close();
}

void Decoder::dumpApproxInstrProfile(uint64_t* approxOpcodeCount, uint32_t numOpcodes)
{
    ...
    for (uint32_t i = 0; i < numOpcodes; i++)
    {
        if (approxOpcodeCount[i])
        {
            ...
            out << std::setw(16) << xed_category_enum_t2str(cat)
                << std::setw(16) << xed_iclass_enum_t2str(iclass)
                << std::setw(32) << xed_iform_enum_t2str(iform)
                << std::setw(16) << approxOpcodeCount[i] << std::endl;
        }
    }
    out.close();
}
